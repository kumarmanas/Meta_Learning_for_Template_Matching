...
class ProtoNet(FewShotModel):
    ...
    def _forward(self, instance_embs, support_idx, query_idx):
        ...
        if True: # self.args.use_euclidean:
            ...
            query1 = query.view(-1, emb_dim).unsqueeze(1) # (Nbatch*Nq*Nw, 1, d)
            proto1 = proto.unsqueeze(1).expand(num_batch, num_query, num_proto, emb_dim)
            proto1 = proto1.contiguous().view(num_batch*num_query, num_proto, emb_dim) # (Nbatch x Nq, Nk, d)
            
            logits1 = - torch.sum((proto1 - query1) ** 2, 2) / self.args.temperature
            #for logits2
            proto2 = F.normalize(proto, dim=-1) # normalize for cosine distance
            query2 = query.view(num_batch, -1, emb_dim) # (Nbatch,  Nq*Nw, d)

            # (num_batch,  num_emb, num_proto) * (num_batch, num_query*num_proto, num_emb) -> (num_batch, num_query*num_proto, num_proto)
            logits2 = torch.bmm(query2, proto2.permute([0,2,1])) / self.args.temperature
