...
class TestCategoriesSampler():
    ...
    def __iter__(self):
        for i_batch in range(self.n_batch):
            ...
            for c in classes:
                l = self.m_ind[c]
                #code added to take only clean template as image to learn as single shot image example
                #1st row of each dataclass belongs to template of that class
                #other rows are scanned/other images which needs to be classified in one of the templates
                a=torch.arange(1,len(l-1))
                a=a[torch.randperm(a.shape[0])]
                initial_value=torch.tensor([0])
                pos=torch.cat((initial_value,a))[:self.n_per]
                #pos = torch.randperm(len(l))[:self.n_per]
                batch.append(l[pos])
            batch = torch.stack(batch).t().reshape(-1)
            yield batch
