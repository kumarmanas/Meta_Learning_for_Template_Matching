...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='ScanImage', choices=['ScanImage'])    
    parser.add_argument('--backbone_class', type=str, default='Res12', choices=['ConvNet', 'Res12','Res50','WRN'])
    ...
    if args.dataset == 'ScanImage':
        # Handle MiniImageNet
        from model.dataloader.scanimage import ScanImage as Dataset 
    else:
        raise ValueError('Non-supported Dataset.')

    trainset = Dataset('train_m', args, augment=True)
    train_loader = DataLoader(dataset=trainset, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=True)
    args.num_class = trainset.num_class
    valset = Dataset('val_m', args)
    ...
    elif 'eff' in args.backbone_class:
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, nesterov=True, weight_decay=0.0005)
    else:
